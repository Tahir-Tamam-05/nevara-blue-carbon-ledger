...
import { Activity, Shield, CheckCircle2, AlertTriangle, Download, Database, Leaf, Link } from 'lucide-react';
...
export default function MRVReport() {
  const [, params] = useRoute('/projects/:id/mrv-report');
  ...
  if (mrvError || mrvData?.status === 'PENDING') {
    return (
      <div className="min-h-screen pt-24 px-6 flex flex-col items-center justify-center text-center">
        ...
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-200 mb-2">
          {mrvData?.status === 'PENDING' ? 'Analysis in Progress' : 'MRV Report Unavailable'}
        </h1>
        <p className="text-slate-500 max-w-md">
          {mrvData?.status === 'PENDING' 
  ...
  const chartData = measurements.map((m: any) => ({
    date: format(new Date(m.measuredAt), 'MMM yyyy'),
    ndvi: m.ndviMean,
    cloud: m.cloudCoverPct
  }));
  ...
  return (
    <div className="min-h-screen pt-24 pb-12 px-6">
      ...
      <div className="max-w-5xl mx-auto space-y-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="bg-white/50 dark:bg-slate-950/50 backdrop-blur-sm border-slate-200 dark:border-slate-800 shadow-sm">
            <CardContent className="p-4 flex flex-col justify-center h-full">
              ...
              <p className="text-xl font-bold text-slate-800 dark:text-slate-200">{score.confidence}</p>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4 flex flex-col justify-center h-full">
              ...
              <div className="flex items-baseline gap-1">
                <p className={`text-xl font-bold ${score.ndviDeltaPct >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                  {score.ndviDeltaPct > 0 ? '+' : ''}{score.ndviDeltaPct.toFixed(1)}%
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/50 dark:bg-slate-950/50 backdrop-blur-sm border-slate-200 dark:border-slate-800 shadow-sm">
            <CardContent className="p-4 flex flex-col justify-center h-full">
              ...
              <p className="text-xl font-bold text-slate-800 dark:text-slate-200">{(score.canopyPct * 100).toFixed(1)}%</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/50 dark:bg-slate-950/50 backdrop-blur-sm border-slate-200 dark:border-slate-800 shadow-sm">
            <CardContent className="p-4 flex flex-col justify-center h-full">
              ...
              <p className="text-xl font-bold text-slate-800 dark:text-slate-200">{score.areaHa} ha</p>
            </CardContent>
          </Card>
        </div>
        ...
        {/* MRV Workflow Visual */}
        {projectData && (
          <MRVWorkflow project={projectData} />
        )}
          ...
          <Card className="border-slate-200 dark:border-slate-800">
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800">
                ...
                <span className="font-mono text-sm">{score.baselineNdvi?.toFixed(4)}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800">
                <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Current NDVI</span>
                <span className="font-mono text-sm">{score.currentNdvi?.toFixed(4)}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800">
                <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Ecosystem Factor</span>
                <span className="font-mono text-sm">{score.ecosystemFactor?.toFixed(2)}x</span>
              </div>
